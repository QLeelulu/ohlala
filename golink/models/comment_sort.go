...
func BuildHtmlString(arrRoots *[]*CommentNode, childCount int, exceptIds string, b *bytes.Buffer, pId int64,
    ...
    for _, item := range *arrRoots {
        ...
        if item.ChildrenCount > 0 && len(item.Children) == 0 {
            ...
            b.WriteString(fmt.Sprintf("<div class='cd' pid='pid%d'><div class='ldmore' lmid='lm%d' ><a href='javascript:' pId='%d' exIds='' pp='%s%d/' tId='%d' lId='%d' srt='%s'>追载(%d)</a></div></div>",
    ...
    rLen := len(*arrRoots)
    if childCount > rLen {
        b.WriteString(fmt.Sprintf("<div class='ldmore' lmid='lm%d' ><a href='javascript:' pId='%d' exIds='%s' pp='%s' tId='%d' lId='%d' srt='%s'>追载(%d)</a></div>",
